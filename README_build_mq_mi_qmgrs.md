**MIQM builder** that also **generates the VIP stack (HAProxy)** and a **Makefile** with `vip-up` / `vip-down` targets (plus a few handy ones). Drop this in as `build_miqm.sh`, run it, and you’ll get:

* `docker-compose.yml` — 1 **active** + 1 **standby** MIQM (shared storage).
* `docker-compose.vip.yml` + `haproxy/haproxy.cfg` — a TCP VIP on one port that always targets the **active** instance.
* `promote_standby.sh` — clean role-swap helper.
* `Makefile` — convenience targets: `up`, `down`, `vip-up`, `vip-down`, `promote`, `status`, `logs`, `clean`.
...
#
# Author      : rob lee (MIQM + VIP fold-in)
...
```bash
chmod +x build_miqm.sh
./build_miqm.sh
make vip-up             # bring up the VIP
make status             # see ACTIVE/STANDBY
make promote            # role swap
make vip-down           # stop/remove the VIP container
make down               # stop MIQM
```

**Prod tips**

* Use **NFSv4** shared storage with reliable locking. Set `NFS_SERVER`/`NFS_EXPORT` env vars when running the script to generate an NFS volume instead of the local bind.
* Keep `CHLAUTH` strict and enable **TLS** for channels outside of throwaway dev.
* For multi-host, pair HAProxy with **keepalived/VRRP** to float a real VIP across hosts (happy to add that if you want).
