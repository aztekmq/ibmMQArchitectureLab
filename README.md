...
This repository describes and automates three containerized IBM MQ architectures for **hands-on learning**, prototyping, and demos. They intentionally favor **clarity over hardening** and are **not intended for production**.
...
* Host OS with `bash` and `ss` (or `netstat`) available
* Images:

  * **MQ base image** for Standalone & MI
...
```
build_mq_qmgrs.sh          # A: Standalone N x QMs
build_mq_mft_qmgrs.sh      # B: MFT domain (Coord, Command, Agents)
build_mq_mi_qmgrs.sh       # C: MIQM Active/Standby; optional VIP artifacts
promote_standby.sh         # C: Controlled promotion (Active ⇄ Standby)
verify_mft.sh              # B: MFT smoke test (REM → LCL, etc.)
verify_mi.sh               # C: MI-only verifier (roles, storage, VIP, failover)
docker-compose.vip.yml     # C (optional): VIP service (generated by builder)
haproxy/haproxy.cfg        # C (optional): VIP config (generated by builder)
Makefile                   # C (optional): convenience targets (generated)
```
...
| Script                 | Role                                    | Key Behaviors                                                                                                                                                                                                                                                                                 | Important Vars                                                                                        | Quick Use                |                        |
| ---------------------- | --------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------- | ------------------------ | ---------------------- |
| `build_mq_mi_qmgrs.sh` | Provision **1 ACTIVE + 1 STANDBY** MIQM | Creates `QM1` on shared storage (bind or NFS); starts **ACTIVE** on `qm1a` (`strmqm QM1`) and **STANDBY** on `qm1b` (`strmqm -x QM1`); defines DEV channel/listener; prints ports. *VIP-integrated variant also generates `docker-compose.vip.yml`, `haproxy/haproxy.cfg`, and a `Makefile`.* | `IMAGE_NAME/TAG`, `QM_NAME`, `PORT_ACTIVE`, `PORT_STANDBY`, `NFS_SERVER`, `NFS_EXPORT` (for NFS mode) | `./build_mq_mi_qmgrs.sh` |                        |
...
* **MI failover doesn’t occur** → Verify NFSv4 with locking; both containers must mount **the same** path; check `AMQERR01.LOG`.
* **2035 (not authorized)** → Lab opens `DEV.APP.SVRCONN`; if you tightened CHLAUTH, grant appropriate auth or use bindings mode for tests.
